       ...
       function selectRandomItems(array, count) {
           ...
           if (GUARANTEED_WINNERS && GUARANTEED_WINNERS.length > 0) {
               ...
               if (GUARANTEED_WINNERS_PER_REQUEST && GUARANTEED_WINNERS_PER_REQUEST > 0) {
                   ...
                   // Create comments for the guaranteed winners
                   for (const winnerName of selectedGuaranteedWinners) {
                       ...
                       if (matchIndex !== -1) {
                           ...
                           // Create a comment with predefined text
       ...
       function displayWinners(winners) {
           ...
           winners.forEach((winner, index) => {
               ...
               if (winner.profilePicture && typeof winner.profilePicture === 'string' && 
                   ...
                   // If no profile picture, display the first letter after @ in the username
                   const username = winner.ownerUsername.replace('@', '');
                   const firstLetter = username.charAt(0).toUpperCase();
                   const letterSpan = document.createElement('span');
                   letterSpan.className = 'avatar-letter';
